$(document).ready(function () {
    ...
    var rfqParts = GetRfqParts();

    var AddOnBucket = function (name, value, margin, waf) {
        var self = this;
    ...
    var SurchargeBucket = function (name, value, margin) {
        var self = this;
        self.bucketName = ko.observable(name);
        self.bucketValue = ko.observable(value);
        self.bucketMargin = ko.observable(margin);
    ...
    var DutyBucket = function (name, value, margin) {
        var self = this;
        self.bucketName = ko.observable(name);
        self.bucketValue = ko.observable(value);
        self.bucketMargin = ko.observable(margin);
    ...
    var CostPart = function (projectPartId, number, weight, usage, addOnTotalPNumber, surchargeTotalValue, dutyTotalValue) {
        var self = this;
        ...
        self.rawCost = ko.observable(0.00);
        self.fixtureCost = ko.observable(0.00);
        self.patternCost = ko.observable(0.00);

        self.annualRawCost = ko.computed(function () {
            var annualRawCost = parseFloat(self.rawCost()) * removeNumberComma(self.costAnnualUsage());
            return parseFloat(annualRawCost).toFixed(2);
        });
        self.costPNumber = ko.computed(function () {
            ...
            return parseFloat(costPNumber).toFixed(2);
        });
        self.machineCost = ko.observable(0.00);
        self.fobCost = ko.computed(function () {
            var fobCost = parseFloat(self.rawCost()) + parseFloat(self.machineCost());
            return parseFloat(fobCost).toFixed(2);
        });
        self.costPartAddOn = ko.computed(function () {
            ...
            return parseFloat(costPartAddOn).toFixed(2);
        });
        self.costPartSurcharge = ko.computed(function () {
            ...
            return parseFloat(costPartSurcharge).toFixed(2);
        });
        self.costPartDuty = ko.computed(function () {
            ...
            return parseFloat(costPartDuty).toFixed(2);
        });
        ...
        self.annualCost = ko.computed(function () {
            ...
            return parseFloat(annualCost).toFixed(2);
        });
    };

    var PricePart = function (projectPartId, number, weight, usage, raw, machine, addOnTotalPNumber, surchargeTotalValue, dutyTotalValue, fixturePrice, patternPrice) {
        var self = this;
        self.projectPartId = ko.observable(projectPartId);
        ...
        self.machinePrice = ko.observable(machine);
        self.addOnTotalPNumber = ko.observable(addOnTotalPNumber);
        self.surchargeTotalValue = ko.observable(surchargeTotalValue);
        self.dutyTotalValue = ko.observable(dutyTotalValue);
        self.fixturePrice = ko.observable(fixturePrice);
        self.patternPrice = ko.observable(patternPrice);
    ...
    var PriceSheetViewModel = function () {
        ...
        self.addBucket = function () {
            if (self.projectMargin() === "") {
                ...
                if (self.bucketType() === "Add-On") {
                    ...
                    self.addOns.push(new AddOnBucket(self.bucketName(), self.bucketValue(), self.projectMargin(), self.waf()));
                    clearAddBucket();
                }
                    ...
                    self.surcharges.push(new SurchargeBucket(self.bucketName(), self.bucketValue(), self.projectMargin()));
                    clearAddBucket();
                }
                else if (self.bucketType() === "Duty") {
                    var profit = self.bucketValue() * (self.projectMargin() * 0.01);
                    var bucketSellValue = (parseFloat(self.bucketValue()) + parseFloat(profit)).toFixed(3);
                    self.duties.push(new DutyBucket(self.bucketName(), self.bucketValue(), self.projectMargin()));
                    clearAddBucket();
                }
                else {
                    $('#alertDiv').html('<div class="alert alert-danger alert-dismissable">' +
        ...
        self.savePriceSheet = function () {
            ...
            $.each(self.surcharges(), function (i, surcharge) {
                console.log(surcharge.bucketName());
            ...
            $.ajax({
                type: 'POST',
                url: "/SouthlandMetals/Operations/Pricing/Create",
                dataType: "json",
        ...
        self.loadCostParts = function () {
            $.each(rfqParts, function (n, part) {
                var costAddOn = parseFloat(part.Weight * self.addOnTotalPNumber()).toFixed(3);
                self.costParts.push(new CostPart(part.ProjectPartId, part.PartNumber, part.Weight, part.AnnualUsage, self.addOnTotalPNumber(), self.surchargeTotalValue(), self.dutyTotalValue()));
            });
        }
        ...
        self.loadPriceParts = function () {
            $.each(self.costParts(), function (n, part) {
                self.priceParts.push(new PricePart(
                     part.projectPartId(), part.costPartNumber(), part.costPartWeight(), part.costAnnualUsage(), part.rawCost() * (1 + self.projectMargin() / 100)
        ...
        self.checkPriceRaw = function (rawPrice, partNumber) {
            $.each(self.priceParts(), function (n, pricePart) {
                if (pricePart.pricePartNumber() == partNumber) {
                    console.log(self.projectMargin())
    ...
    function GetRfqParts() {
        var costParts;
        $.ajax({
            type: 'GET',
            url: "/SouthlandMetals/Operations/Pricing/GetPartsForPriceSheet",
            dataType: "json",
            async: false,
            data: { "rfqId": rfqId, "includeRaw": includeRaw, "includeMachined": includeMachined },
            success: function (result) {
                costParts = result.OrderParts;
            },
            error: function (err) {
                console.log('Error ' + err.responseText);
            }
