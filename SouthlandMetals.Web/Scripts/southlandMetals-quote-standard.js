$(document).ready(function () {
                ...
                if (existingPart.length > 0) {
                    ...
                    if (invalidPart) {
                        ...
                        parts.push({
                            ...
                            'PartDescription': partDescription, 'CustomerId': addPartCustomerId, 'FoundryId': addPartFoundryId, 'Weight': weight, 'AnnualUsage': annualUsage, 'Price': price, 'Cost': cost,
    ...
    CreateQuote = function () {
        ...
        if (parts.length < 1) {
            ...
            if (!$("#standardQuoteForm")[0].checkValidity()) {
                $('#alertDiv').html('<div class="alert alert-danger alert-dismissable">' +
                               '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>' +
                               '<strong>Warning!</strong>&nbsp;Please fill in all required fields!</div>');
                $('#standardQuoteForm input[required]').each(function () {
                    if ($(this).val() === "") {
                        $(this).addClass("form-error");
                    }
                });

                $('#standardQuoteForm select[required]').each(function () {
                    if ($(this).val() === "") {
                        $(this).addClass("form-error");
                    }
                });

                $('#standardQuoteForm textarea[required]').each(function () {
                    if ($(this).val() === "") {
                        $(this).addClass("form-error");
                    }
                });
    ...
    $('#customerAddressId').change(function () {
        ...
        $.ajax({
            type: 'GET',
            url: "/SouthlandMetals/Administration/Customer/GetCustomerByAddress",
